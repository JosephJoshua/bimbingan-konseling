...
const form = useForm({
  ...
  consultation_category_id: props.categories.at(0)?.id ?? null,
  description: '',
});
...
const submit = () => {
  ...
  form.post(
    route('students.consultations.store', { student: props.student.id }),
  );
};
...
<template>
  <Head :title="`${student.full_name} - Tambah Konsultasi`" />
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center gap-4">
        ...
        >
          {{ student.full_name }} | Tambah Konsultasi
        </h2>
      </div>
    </template>
